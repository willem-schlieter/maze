...
class Maze:
    def __init__ (self, model: Model):
        ...
        self.x = model.x;
        self.y = model.y;
        self.grössen = ((style.S_FELD + style.S_WAND) * self.x + style.S_WAND, (style.S_FELD + style.S_WAND) * self.y + style.S_WAND);
    ...
    def render (self) -> pg.Surface:
        ...
        # FOOD
        for f in self.model.food: pg.draw.rect(self.surface, style.F_FOOD, (*feld2pos(f), style.S_FELD, style.S_FELD));

        if (self.stein != self.model.exit):
            # Spielstein
            pg.draw.rect(self.surface, style.F_ST, (*feld2pos(self.stein), style.S_FELD, style.S_FELD));
            # Ausgang
            pg.draw.rect(self.surface, style.F_EX, (*feld2pos(self.model.exit), style.S_FELD, style.S_FELD));

        return self.surface;

    def step (self, dir: int) -> List[bool]:
        result = [False, False, False]; # [gegangen, gegessen, exit]
        if not self.model.walls(*self.stein)[dir]:
